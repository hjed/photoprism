...
func XMPMedia(fileName string, fileType fs.FileFormat) (data Data, err error) {
	err = data.XMPMedia(fileName, fileType)

	return data, err
}

// Parses XMP data out of a media file
func (data *Data) XMPMedia(fileName string, fileType fs.FileFormat) (err error) {
	defer func() {
		if e := recover(); e != nil {
...
func Parse(rs io.ReadSeeker, size int) (ec *jpegstructure.SegmentList, err error) {
	defer func() {
		if state := recover(); state != nil {
			err = fmt.Errorf("metadata: %s (xmp panic)\nstack: %s", err, debug.Stack())
		}
	}()
