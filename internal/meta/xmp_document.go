...
type XmpDocument struct {
	...
	RDF     struct {
		...
		Description struct {
			...
			Rating          string `xml:"Rating,attr"`      // 4
			...
			DateCreatedAttr string `xml:"DateCreated,attr"` // 2020-01-01T17:28:25.72962...
...
func (doc *XmpDocument) TakenAt() time.Time {
	...
	if s == "" {
		s = SanitizeString(doc.RDF.Description.DateCreatedAttr)
		if s == "" {
			return taken
		}
	}
	...
	log.Debugf("parsed taken at as '%s' from '%s'", taken.Format(time.RFC3339), s)
